...
class Feedback(db.Model):
    ...
    answer_method = db.Column(db.String(20), nullable=True)  # 'email', 'platform'
    ...
    # Связи для удобства (необязательно, только если хочешь быстро вытаскивать имена админов)
    user = db.relationship('User', foreign_keys=[user_id], backref='feedbacks_sent', lazy=True)
    admin = db.relationship('User', foreign_keys=[answered_by], backref='feedbacks_answered', lazy=True)
